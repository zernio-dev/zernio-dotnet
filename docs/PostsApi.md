...
<a id="createpost"></a>
# **CreatePost**
> CreatePost200Response CreatePost (CreatePostRequest createPostRequest, string? idempotencyKey = null, Guid? xRequestId = null)
...
namespace Example
{
    public class CreatePostExample
    {
        public static void Main()
        {
            ...
            var xRequestId = "xRequestId_example";  // Guid? | Optional client-generated request identifier. A request with the same value as an earlier one within 24 hours that also matches its content fingerprint (same account, content and media URLs) returns the original post (HTTP 200) instead of a duplicate-content 409. It does not protect a retry whose body differs; use `Idempotency-Key` for that. SDKs set it automatically. See the operation description for the full idempotency contract.  (optional) 

            try
            {
                // Create post
                CreatePost200Response result = apiInstance.CreatePost(createPostRequest, idempotencyKey, xRequestId);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
...
{
    // Create post
    ApiResponse<CreatePost200Response> response = apiInstance.CreatePostWithHttpInfo(createPostRequest, idempotencyKey, xRequestId);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}
...
| **xRequestId** | **Guid?** | Optional client-generated request identifier. A request with the same value as an earlier one within 24 hours that also matches its content fingerprint (same account, content and media URLs) returns the original post (HTTP 200) instead of a duplicate-content 409. It does not protect a retry whose body differs; use &#x60;Idempotency-Key&#x60; for that. SDKs set it automatically. See the operation description for the full idempotency contract.  | [optional]  |
...
| Status code | Description | Response headers |
|-------------|-------------|------------------|
| **200** | Nothing was created. Either a dryRun preview (TikTok only), which deliberately carries no numeric cap detail, only a per-account go/no-go and a reason; or a retry matched by &#x60;Idempotency-Key&#x60; or &#x60;x-request-id&#x60;, which returns the original post (&#x60;PostCreateResponse&#x60;, with &#x60;message: \&quot;Post already exists (idempotent retry)\&quot;&#x60;).  A newly created post is always a 201.  |  -  |
| **201** | Post created |  -  |
...
namespace Example
{
    public class ListPostsExample
    {
        public static void Main()
        {
            ...
            var accountId = "accountId_example";  // string? | Filter posts to those published via a specific account (24-char hex ObjectId). (optional) 
...
| **accountId** | **string?** | Filter posts to those published via a specific account (24-char hex ObjectId). | [optional]  |
