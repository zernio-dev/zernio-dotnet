...
namespace Example
{
    public class CreatePostExample
    {
        public static void Main()
        {
            ...
            var xRequestId = "xRequestId_example";  // Guid? | Optional client-generated request identifier for safe retry (idempotency). When two requests carry the same value, the second is treated as a retry of the first and returns the original post (HTTP 200) instead of creating a duplicate. Window is ~5 minutes from the first request. Generate a UUID per logical call. SDKs do this automatically; HTTP clients should set it themselves or omit it. See the operation description for the full idempotency contract.  (optional) 
...
| **xRequestId** | **Guid?** | Optional client-generated request identifier for safe retry (idempotency). When two requests carry the same value, the second is treated as a retry of the first and returns the original post (HTTP 200) instead of creating a duplicate. Window is ~5 minutes from the first request. Generate a UUID per logical call. SDKs do this automatically; HTTP clients should set it themselves or omit it. See the operation description for the full idempotency contract.  | [optional]  |
...
| Status code | Description | Response headers |
|-------------|-------------|------------------|
| **200** | A dryRun preview (TikTok only): nothing was created. Deliberately carries no numeric cap detail, only a per-account go/no-go and a reason.  The schema is a union only so generated clients can type both success shapes of this operation: a 200 is always the dry-run verdict, and a created post is always a 201.  |  -  |
| **201** | Post created |  -  |
