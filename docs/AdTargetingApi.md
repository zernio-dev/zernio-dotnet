...
namespace Example
{
    public class SearchAdTargetingExample
    {
        public static void Main()
        {
            ...
            var dimension = "geo";  // string? | What to search. `geo` resolves locations (scope further with `geoType`), `interest`/`behavior` resolve audience entities, `income` resolves income-tier options, `workPosition`/`workEmployer`/`workIndustry` resolve Meta work demographics. Defaults to `interest` for backward compatibility with the deprecated /v1/ads/interests alias. (optional)  (default to interest)
